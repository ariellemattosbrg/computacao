...
dataset['ALVO_BI']   = [0 if np.isnan(x) or x > 120 else 1 for x in dataset['TEMP_RECUPERACAO']]
...
dataset['ADULTO']           = [1 if x > 29 and x < 41  else 0 for x in dataset['IDADE']] # Trata pessoas entre 30 e 40 anos
dataset['PRE_SEXO_F']       = [1 if x=='F'                         else 0 for x in dataset['CD_SEXO']]
dataset['RENEGOCIACAO']     = [1 if x=='RENEGOCIAÇÃO'              else 0 for x in dataset['TIPO_EMPRESTIMO']]
dataset.to_csv("Case_cobranca_updated.csv")
...
##------------------------------------------------------------
y = dataset['ALVO_BI']              # Carrega alvo ou dataset.iloc[:,7].values
...
print()
print('---------------------------------------------')
print('Regressão Linear - Erro de Classificação:',Erro_RL_Classificacao)
print('Regressão Linear - MSE:',Erro_RL_MSE)
print('----------------------------------------------')
...
print('Regressão Logística - MSE:',Erro_RLOG_MSE)
print('----------------------------------------------')

Erro_AD_Classificacao = np.mean(np.absolute(df_test['CLASSIF_DT'] - df_test['TARGET']))
Erro_AD_MSE = np.mean((df_test['REGRESSION_DT'] - df_test['TARGET']) ** 2) 
print()
print('---------------------------------------------')
print('Regressão Árvore de Decisão - Erro de Classificação:',Erro_AD_Classificacao)
print('Regressão Árvore de Decisão - MSE:',Erro_AD_MSE)
print('----------------------------------------------')
